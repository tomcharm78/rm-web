...
import { CheckCircle2, Plus, Loader2, ThumbsUp, ThumbsDown, Inbox, FileText, Paperclip, Pencil, Trash2 } from 'lucide-react';
import { useAuth } from '@/providers/auth-provider';
import { useLanguage } from '@/providers/language-provider';
import {
  listApprovalsForApprover, listMyApprovalRequests, decideApprovalRequest,
...
import { APPROVAL_STATUS_LABELS, approvalStatusColor, type ApprovalRequest, type ApprovalStatus } from '@/types/approval';
...
export default function ApprovalsPage() {
  ...
  const inboxStatus: ApprovalStatus = tab === 'mine' ? 'pending' : (tab as ApprovalStatus);
  const inboxQ = useQuery({
    queryKey: ['approvals-inbox', tab],
    queryFn: () => listApprovalsForApprover(inboxStatus),
    enabled: tab !== 'mine',
  });
  ...
  const rows = tab === 'mine' ? (mineQ.data ?? []) : (inboxQ.data ?? []);
  const loading = tab === 'mine' ? mineQ.isLoading : inboxQ.isLoading;

  // attachment counts for the visible requests (one batch query, for the paperclip)
  const countsQ = useQuery({
    queryKey: ['approval-attachment-counts', rows.map((r) => r.id)],
    queryFn: () => getAttachmentCounts('approval', rows.map((r) => r.id)),
        ...
        {tabs.map((t) => (
          <button key={t.key} onClick={() => { setTab(t.key); setExpanded(null); }}
            style={{
              ...
              background: tab === t.key ? 'hsl(var(--foreground))' : 'transparent',
              color: tab === t.key ? 'hsl(var(--background))' : 'hsl(var(--foreground))',
            }}>
